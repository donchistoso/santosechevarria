# Cover letter template — Santos Echevarria

How to use: copy this file to `cover-letters/<dealership-slug>.md`, fill the
`{{...}}` slots, cut anything that doesn't fit on one page. Never invent a
number to fill a slot — if there's no real figure, keep the general phrasing.

Tone check: a dealership GM reads this in under a minute, standing up. Short
paragraphs. Concrete. No corporate throat-clearing. He is a closer with 33
years of receipts — the letter should sound like that, not like a template.

---

Santos Echevarria
Miami, FL · (786) 554-0173 · santosechevarria1@gmail.com

{{DATE}}

{{HIRING MANAGER NAME, or "Hiring Manager"}}
{{DEALERSHIP NAME}}
{{CITY, FL}}

Dear {{NAME}},

**Opening — who he is in one line, and why this store.**
I've spent 30 years in South Florida F&I offices, the last 28 of them at Bill
Seidle Nissan. I'm writing about the {{ROLE}} opening at {{DEALERSHIP}} because
{{SPECIFIC REASON — brand, volume, ownership group, reputation, growth, a
person he knows there}}.

**Middle — the proof.** Pick two or three, not all of them:
- Six-time Nissan Award of Excellence winner and Platinum in Security Plus
  products every year since 2005 — the manufacturer has been grading my
  numbers for two decades.
- I work the whole credit spectrum. Multiple lender relationships built over
  years, which is what turns a hard deal into a funded one.
- I supported a 15-person sales floor and trained them on how to hand a
  customer to the finance office without losing the customer or the gross.
- Clean compliance record across thousands of deals — TILA, Red Flags, OFAC.
- CDK, DealerTrack, Reynolds & Reynolds. No ramp-up on your DMS.

**Close — ask for the conversation.**
{{ONE LINE ON WHAT HE'D BRING TO THIS SPECIFIC STORE.}} I'd welcome the chance
to sit down and go through my numbers with you. I can be reached at
(786) 554-0173.

Sincerely,
Santos Echevarria

---

## Why he's leaving — use this

Bill Seidle's is eliminating the finance department. That is the answer on
every application and in every interview:

> "The dealership is eliminating the finance department, so I'm looking for my
> next opportunity after 28 years there."

Blameless, explains the timing, and invites no follow-up. Never use the older
"the work had become predictable" framing.

## Slot notes

| Slot | Where to get it |
|---|---|
| `{{SPECIFIC REASON}}` | Dealer website, Google reviews, DealerRater, who owns the group, recent expansion. One sentence of real homework beats a page of adjectives. |
| Numbers | `_source/facts-of-record.md` only. Pending Santos. |
| `{{ROLE}}` | Match the posting's exact title. |
