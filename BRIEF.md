# Brief — 2026-09-05

## State
Initiative stood up in its own private repo. All four workstreams have a
first draft or a scaffold: two resume variants, a cover-letter template,
complete LinkedIn copy plus a setup runbook, and job-search tracking.

Nothing has been sent to anyone. Nothing is live.

## Live now
- **Resume, FM variant** (`resume/resume-finance-manager.md`) — carried over
  from the prior session's selected v1. Ready to export once Santos confirms.
- **Resume, F&I Director variant** (`resume/resume-fi-director.md`) — new
  draft, unreviewed. Read the "Honest gap to close" note at the bottom.
- **LinkedIn** (`linkedin/`) — copy is written for every field. The account
  itself doesn't exist; Santos has to create it.
- **Cover letter — THE COLLECTION** (`cover-letters/the-collection-fi-manager.md`)
  — draft **v3**, strengths-only, mapped to the posting's own three section
  headings. **Needs his review before submission.**

- **Résumé — THE COLLECTION variant** (`resume/resume-the-collection.md`) —
  third variant, tailored to the posting's exact title, vocabulary and
  responsibility order. This is the one that goes with the application.
- **Exports** — all three variants rendered to one-page PDFs in `exports/`.
  Build with `./_build/build.sh`.

## Open application
**THE COLLECTION — Finance & Insurance Manager**, Coral Gables. Cover letter
required. Not yet submitted. Full posting + fit analysis is in
`_source/postings/the-collection-fi-manager.md`.

Letter draft **v3** is strengths-only. The posting's five-year luxury minimum
is deliberately not addressed in writing — Santos handles it in the interview
if it comes up, and his store does carry high-line units. The letter opens on
the 33-year record, mirrors the posting's three section headings, and leans on
manufacturer-verified award numbers.

**Before it goes:** Santos reads it; confirm he's available for
evenings/weekends/holidays/events (the letter asserts it); prep his answer for
the luxury-experience question, since it will now come up live.

## Parked
- DOCX export. Only needed if a posting's ATS refuses PDF.
- Target dealership list. Empty on purpose — build it *with* Santos.

## Needs a decision / needs Santos
1. **Real numbers.** PVR, penetration %, CSI, deal volume, F&I gross. Every
   resume bullet currently uses general language because of it.
2. **Did he ever supervise other F&I managers?** Decides whether the director
   variant works for group roles or only single-point stores.
3. **Nissan Award of Excellence years.** 6 wins, 2020–2026 range. Which years?
4. **Spanish fluency?** Material in the Miami market — belongs in the headline
   if yes.
5. **Education / licensing / AFIP or NADA training?** Currently omitted.
6. **Commute radius, and Nissan-Honda vs. brand-agnostic.**

Full list: `_source/facts-of-record.md`.

## Blocked
GitHub App can't create repos in this session (403). **Juan: create an empty
private repo named `santos-echevarria-career` — no README, no .gitignore —
and the local commit is ready to push.**
