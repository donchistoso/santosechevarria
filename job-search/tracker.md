# Application tracker

**Filling out a form?** Use `APPLICATION-ANSWERS.md` — every recurring field
and essay question is pre-answered there. Copy-paste instead of retyping, and
his answers stay consistent across stores in the same group.

One row per application. Update the date columns as things move — the point is
that follow-up never gets dropped.

| # | Dealership | Role | Source | Applied | Resume variant | Cover letter | Contact | Last touch | Status | Notes |
|---|---|---|---|---|---|---|---|---|---|---|
| 1 | THE COLLECTION | Finance & Insurance Manager | Hireology (job 2433980) · found via LinkedIn | *not yet* | THE COLLECTION (tailored) | `the-collection-fi-manager.md` | Hiring Manager (name unknown) | 2026-09-05 | `researching` | Luxury group, 8 brands, Coral Gables. Cover letter required. Comp ~$76–136K per Glassdoor est. Awaiting Santos's review. |

**Status values:** `researching` · `applied` · `phone screen` · `interview` ·
`offer` · `declined` · `no response` · `closed`

**Follow-up rule:** no reply after 5 business days → one call to the store and
ask for the GM or GSM by name. Dealership hiring runs on the phone, not on
application portals.
| 2 | EchoPark Automotive (Sonic) | Automotive Finance Manager / "Finance Coach" | LinkedIn (job 4462064909) | *not yet* | Finance Manager (general) | `echopark-orlando-fi-manager.md` | Hiring Manager | 2026-09-07 | `researching` | **Orlando — relocation, family there.** New store opens Dec 2026, opening-team hire. $90–110K. Used-car one-price model. LinkedIn flags low fit (location). |
